...
def save_rating(rating, username=None, movie_title=None, user_id=None, movie_id=None):
    try:
        ...
        if movie_id:
            ...
            return Response(
                {"message": "please send either movie_id or movie_title"},
        ...
        if rating:
            rating_obj = Rating.objects.create(
                movie=movie_obj, rating=rating, user=user_obj
            )
            json_to_send = RatingSerializer(rating_obj).data
            return Response({"data": json_to_send})
        else:
            return Response(
                {"message": "please send rating for the movie"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    except Exception as e:
        return Response(
            {"message": f"Failed to save ratings due to {e}"},
...
def getMovieRatingByName(request, movie_title):
    ...
    try:
        ...
        print(e)
        return Response(
            {"message": "unable to fetch movie rating"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def onScreenApiView(request, pincode):
    ...
    """
    active_movies = Onscreen.objects.filter(
        pincode=pincode, status=MovieStatusTypes.ACTIVE.value
    )
