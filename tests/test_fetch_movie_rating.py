...
class TestFetchMovieRatingByName(TestCase):
    def setUp(self):
        ...
        self.rating_obj = Rating.objects.create(
            movie=self.movie, user=self.user, rating=self.rating
        )
        return super().setUp()

    def test_fetchMovieRatingByName(self):
        response = self.client.get(
            reverse("getMovieRatingByName", kwargs={"movie_title": self.movie.name})
        )
...
class TestFetchMovieRatingByID(TestCase):
    ...
    def test_fetchMovieRatingById(self):
        response = self.client.get(
            reverse("getMovieRatingById", kwargs={"movie_id": self.movie.id})
        )
        self.assertEquals(response.status_code, 200)
        json_data = json.loads(response.content)
        count = json_data.get("count")
