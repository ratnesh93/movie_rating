...
class TestActiveMovies(TestCase):
    def setUp(self):
        ...
        self.pincode = 110001
        self.movie = Movie.objects.create(name="testMovie", year="2022")
        self.onScreen = Onscreen.objects.create(pincode=self.pincode, movie=self.movie)
        self.movie2 = Movie.objects.create(name="testMovie2", year="2022")
        self.onScreen2 = Onscreen.objects.create(
            pincode=self.pincode, movie=self.movie2
        )
        return super().setUp()

    def test_fetch_active_movies_by_pincode(self):
        response = self.client.get(
            reverse("onScreenApiView", kwargs={"pincode": self.pincode})
        )
